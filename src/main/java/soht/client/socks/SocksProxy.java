...
import soht.client.core.Resolver;
...
public class SocksProxy extends Proxy {
	 ...
	 * @param socksSocket
	 */
	public SocksProxy(ConfigurationManager configurationManager, Host host, Socket socksSocket) {
		super(configurationManager, host);

		this.host = host;
		this.configurationManager = configurationManager;
		this.socksSocket = socksSocket;
		this.resolver = new Resolver(configurationManager);
	 ...
	 */
	public SocksProxy(ConfigurationManager configurationManager, Socket socksSocket) throws ConfigurationException {
	...
	public void run() {
		try {
			...
			// do anything else
			DataInputStream inputStream = new DataInputStream(socksSocket.getInputStream());
			DataOutputStream outputStream = new DataOutputStream(socksSocket.getOutputStream());
	 ...
	 * @throws Exception
	 */
	private void commonProxyConnect() throws Exception {
		...
		if (configurationManager.isUseStatelessConnection()) {
			...
			BaseProxy proxy = new ProxyReadWrite(getName() + "-ReadWrite", configurationManager, connectionId,
					socksSocket);
		...
		// Use the normal stateful read connection.
			...
			BaseProxy readProxy = new ProxyReader(getName() + "-Reader", configurationManager, connectionId,
					socksSocket);
			readProxy.start();

			BaseProxy writeProxy = new ProxyWriter(getName() + "-Writer", configurationManager, connectionId,
					socksSocket);
			writeProxy.start();
		}
	}
	 ...
	 * @throws Exception
	 */
	private void v4Connect(DataInputStream inputStream, DataOutputStream outputStream) throws Exception {
	 ...
	 * @throws Exception
	 */
	private void v4Run(DataInputStream inputStream, DataOutputStream outputStream) throws Exception {
	 ...
	 * @throws IOException
	 */
	private void v4SendErrorResponse(DataOutputStream outputStream, int code) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void v4SendResponse(DataOutputStream outputStream, int code, int port, InetAddress address)
			throws IOException {
	 ...
	 * @throws Exception
	 */
	private void v5Connect(DataInputStream inputStream, DataOutputStream outputStream, InetAddress address,
	 ...
	 * @throws IOException
	 */
	private InetAddress v5GetInetAddress(DataInputStream inputStream) throws IOException {
		...
		case V5_ADDR_DOMAINNAME:
			...
			address = resolver.resolve(domainName);
			break;
		}
	...
	/**
	 * Negotiate the authentication method with the client. Return a boolean
	 * indicating whether or not the negotiation was successful.
	 * 
	 * @param inputStream
	 * @param outputStream
	 * @return
	 * @throws IOException
	 */
	private boolean v5NegotiateAuthentication(DataInputStream inputStream, DataOutputStream outputStream)
			throws IOException {
	 ...
	 * @throws Exception
	 */
	private void v5Run(DataInputStream inputStream, DataOutputStream outputStream) throws Exception {
	 ...
	 * @throws IOException
	 */
	private void v5SendCommandNotSupported(DataOutputStream outputStream) throws IOException {
		byte[] response = new byte[] { (byte) 0x05, (byte) V5_REPLY_COMMAND_NOT_SUPPORTED, (byte) 0x00 };
		outputStream.write(response);
		outputStream.flush();
		socksSocket.close();
	}
